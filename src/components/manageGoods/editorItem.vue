<template>
    <div class="addItem">
        <div class="goodsItem">
            <figure>
                <section>
                    ...
                    <p>
                        <span>销售价格</span>
                        <input type="tel" maxlength="11" placeholder="请输入销售价格" v-model="price" @keyup="priceFn" @focus="$store.state.buttonShow=false" @blur="$store.state.buttonShow=true">
                    </p>
                    <p>
                        <span>单位</span>
                        <input type="text" placeholder="请输入单位" maxlength="5" v-model="unit" disabled>
                    </p>
                    <p>
                        <span>有效期</span>
        ...
        <button @touchend="checkInputDataFn" v-show="$store.state.buttonShow">确定</button>
    </div>
</template>
...
//                    console.log(d)
            ...
            confirmFn(){
                ...
                let dataPost={
                    ...
                    'goodsDesc':_this.goodsDesc,
                    'useExplain':_this.useExplain,
                    'state':_this.putaway?1:0
                }
                this.$http({
                    method:'POST',
                    url:`${_this.$URL}api/v1/store/goods/update`,
                    headers:{
...
//                    console.log(d)
                    if (d.code === 0){
                        ...
                        _this.$store.state.errorMsg='<span style="color:#4CCF6F;">编辑成功</span>'
                        _this.$store.state.errorStatus=true
                        setTimeout(()=>{
                            _this.$store.state.errorStatus=false
                            _this.$store.state.errorMsg=''
                            _this.$router.push('/manageGoods')
                        },1500)
                    }else {
                        _this.$store.state.errorMsg=d.msg
                        _this.$store.state.errorStatus=true
...
//                    console.log(d)
                    if (d.code === 0){
                        _this.goodsImage=d.data.goodsImage.split('.com/')[1]
