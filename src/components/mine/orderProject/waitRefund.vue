            ...
            <div class="myOrderLists">
                <div class="orderList" v-for="opd in orderProjectData" :key="opd.orderId">
                    ...
                    <section class="status">
                        <p>
                            <span class="payment" @touchend="handleOrder(opd.orderId)">同意退款</span>
                        </p>
                    </section>
                </div>
            </div>
...
<script>
    ...
    export default {
        ...
        created() {
            this.waitRefundFn(this.pageNo)
        },
        computed: {},
        watch: {},
        methods: {
            ...mapActions(['loadingShowA']),
            waitRefundFn(pn,fun){ // 获取列表
                let _this=this
                if (!fun){
                    _this.loadingShowA()
                }
...
//                    console.log(d)
                    if (d.code === 0){
                        if(pn===1){
                            ...
                            if(d.data.list.length===0){
                                _this.$store.state.reminderWords='还没有待退款信息'
            ...
            confirmOrder(){ // 点击同意退款
                ...
                this.$http({
                    method:'POST',
                    url:`${_this.$URL}api/v1/store/goods/order/agree/refund`,
                    headers:{
                        accessToken:_this.$store.state.STOREINFO.ACCESSTOKEN
                    },
                    params:{
            ...
            pullDownFn(){
                ...
                this.waitRefundFn(this.pageNo,this.resetFn)
            },
            pullUpFn(){
                // 上拉加载
                this.pageNo++
